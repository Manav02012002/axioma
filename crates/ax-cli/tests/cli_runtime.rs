...
fn run_command_executes_file() {
    let out = bin()
        .args([
            "run",
            repo_file("examples/equation_manipulation.ax")
                .to_string_lossy()
                .as_ref(),
        ])
        .output()
...
fn tableau_render_with_slots_matches_exact_stdout() {
    let out = bin()
        ...
        .output()
        .expect("run tableau render with slots");
...
#[test]
fn tableau_render_without_slots_matches_exact_stdout() {
    let out = bin()
        .args(["tableau", "render", "--shape", "3,1"])
        .output()
...
#[test]
fn tableau_summary_prints_tensor_symmetry_summary() {
    let out = bin()
        .args([
            "tableau",
            "summary",
    ...
    assert!(String::from_utf8_lossy(&out.stdout).contains("tableau[0]:"));
}
...
fn run_qm_tutorial_executes_end_to_end() {
    let out = bin()
        ...
        .args([
            "run",
            repo_file("examples/qm_tutorial.ax")
                .to_string_lossy()
                .as_ref(),
        ])
        .output()
...
fn run_gr_tutorial_executes_end_to_end() {
    let out = bin()
        ...
        .args([
            "run",
            repo_file("examples/gr_tutorial.ax")
                .to_string_lossy()
                .as_ref(),
        ])
        .output()
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("imported std.conventions.mtw"),
        "stdout:\n{stdout}"
    );
...
fn run_qft_examples_execute_end_to_end() {
    for rel in ["examples/brst_qed.ax", "examples/wess_zumino_model.ax"] {
        ...
        if rel.ends_with("brst_qed.ax") {
            ...
            assert!(
                stdout.contains("initialized N=1 superspace"),
                "stdout:\n{stdout}"
            );
...
fn run_qft_std_modules_execute_end_to_end() {
    for rel in [
        ...
        match rel {
            ...
            "std/qft/normal_ordering.ax" => {
                assert!(
                    stdout.contains("creation(a)annihilation(a)"),
                    "stdout:\n{stdout}"
                );
...
fn run_kerr_newman_module_exposes_full_metric_matrix() {
    let out = bin()
        ...
        .args([
            "run",
            repo_file("std/gr/kerr_newman.ax")
                .to_string_lossy()
                .as_ref(),
        ])
        .output()
...
fn run_cosmology_perturbation_module_exposes_demo_bindings() {
    ...
    assert!(
        stdout.contains("second_order_00_constraint"),
        "stdout:\n{stdout}"
    );
    assert!(stdout.contains("H_star"), "stdout:\n{stdout}");
...
fn run_differential_forms_module_exposes_forms_operations() {
    ...
    assert!(
        stdout.contains("∂x²/∂x") || stdout.contains("2x"),
        "stdout:\n{stdout}"
    );
    assert!(stdout.contains("[-1Q, P]"), "stdout:\n{stdout}");
    assert!(
        stdout.contains("M") && stdout.contains("[0"),
        "stdout:\n{stdout}"
    );
}
...
fn run_qm_spin_module_exposes_pauli_commutator() {
    let out = bin()
        ...
        .args([
            "run",
            repo_file("std/qm/spin.ax").to_string_lossy().as_ref(),
        ])
        .output()
...
fn run_qm_bell_module_exposes_reduced_density_matrix() {
    let out = bin()
        ...
        .args([
            "run",
            repo_file("std/qm/bell.ax").to_string_lossy().as_ref(),
        ])
        .output()
...
fn run_qm_harmonic_oscillator_module_exposes_abstract_oscillator_algebra() {
    ...
    assert!(
        stdout.contains("√2number_state(ho, 2)"),
        "stdout:\n{stdout}"
    );
    assert!(
        stdout.contains("1 + creation(ho)annihilation(ho)"),
        "stdout:\n{stdout}"
    );
    assert!(
...
fn fix_command_repairs_missing_semicolon() {
    ...
    assert_eq!(
        fs::read_to_string(&src).expect("read fixed"),
        "module demo;"
    );
    let parsed: serde_json::Value =
...
fn repl_help_and_quit_work() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("Axioma REPL commands:"),
        "stdout:\n{stdout}"
    );
    assert!(
        stdout.contains(":latex             Toggle LaTeX input mode"),
        "stdout:\n{stdout}"
    );
}
...
fn repl_meta_commands_execute_end_to_end() {
    ...
    assert!(
        stdout.contains("Expression pool enabled."),
        "stdout:\n{stdout}"
    );
    assert!(stdout.contains("Unique pooled nodes:"), "stdout:\n{stdout}");
    assert!(
        stdout.contains("Parallel mode enabled."),
        "stdout:\n{stdout}"
    );
    assert!(
        stdout.contains("Parallel mode disabled."),
        "stdout:\n{stdout}"
    );
    ...
    assert!(
        stdout.contains("Suggested algorithms:"),
        "stdout:\n{stdout}"
    );
    ...
    assert!(
        stdout.contains("metric_signature: MostlyPlus"),
        "stdout:\n{stdout}"
    );
